...
def recommend():
    global global_recommended_products
    ...
    # Get top N most similar products based on cosine similarity for specific skin type and category
    ...
    # Get product IDs from the filtered products
    ...
    top_n_indices = np.argsort(hybrid_scores.flatten())[::-1]
    ...
    # Simpan rekomendasi ke variabel global
    global_recommended_products = top_n_products_info
...
def filter_recommendations():
    global global_recommended_products
    form_data = request.form
    show_option = form_data.get('show_option')

    if show_option == 'show_top_5':
        filtered_recommendations = global_recommended_products[:5]
    elif show_option == 'show_all':
        filtered_recommendations = global_recommended_products
    else:
        filtered_recommendations = global_recommended_products
